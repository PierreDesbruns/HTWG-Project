...
for row in range(0, nbRows):
    for column in range(0, nbColumns):
        ...
        #pca_datas_transformed[row, column] = (np.dot(cov_changeBasis[:,row], datas_standardized[:,column])) * cov_eigVal_modified[row]
...
for row in range(0, nbRows):
    for column in range(0, nbColumns):
        ...
        #error_matrix[row, column] = (pca_datas[row, column] - datas_standardized[row, column])**2
...
#print(np.matmul(cov_changeBasis, np.matmul(np.diag(cov_eigVal_modified/cov_matrix_trace), np.matmul(cov_changeBasis_inv,datas_standardized))))
#print(np.matmul(cov_changeBasis, np.matmul(np.diag(cov_eigVal_modified), np.matmul(cov_changeBasis_inv,datas_standardized))))
#print(pca_datas_standardized)
